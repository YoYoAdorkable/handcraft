...
from hasset.models import Asset, BisGroup, IDC, Adapter
from hmonit.models import Monit, IP_Status, IP_Alarm_Status, WEB_Status, WEB_Alarm_Status, Dnspod, Dnspod_Alarm, Port, Port_Ip_Alarm_Status, Port_Alarm_Status
from hlog.models import Log, WEB_Logs
from hasset.models import AssetAlias
...
CONF = ConfigParser()
CONF.read(os.path.join(BASE_DIR, 'handcraft.conf'))
...
def user_perm_group_api(username):
    if username:
        ...
        for user_group in user_group_all:
            perm_list.extend([user_group])
             
        
        asset_group_list = []
        for perm in perm_list:
            asset_group_list.append([perm])
        return asset_group_list

def monit_perm_list_api(username):
    #user = User.objects.get(username=username)
    #group = UserGroup.objects.get(id=id)
    monit_list = []
    group = username.group.all()
    name = UserGroup.objects.get(name=group[0])
    id = Monit.objects.filter(contact=name) 
    monit_list.extend(id)
    return monit_list
...
def monit_perm_api(monit):
    if monit:
        monit_list = []
        monit_group_all = monit.contact.all()
        for monit_group in monit_group_all:
            monit_list.extend([monit_group])


        user_permed_list = []
        for user_group in monit_list:
...
def node_auth(request):
    ...
    if not user:
        auth = 0
    if not filename.startswith('/opt/handcraft/logs/connect/'):
        auth = 0
    if auth:
