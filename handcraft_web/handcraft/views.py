...
def index(request):
    ...
    if is_common_user(request):
        ...
        monits = Monit.objects.all()
        #status = IP_Alarm_Status.objects.all()
        status = IP_Status.objects.all()
        alarm = WEB_Status.objects.all()
        dnspods = Dnspod.objects.all()
        dnspod = Dnspod_Alarm.objects.all()
        ports = Port.objects.all()
        port_ip = Port_Ip_Alarm_Status.objects.all()
        ...
        active_monits = Monit.objects.filter(rule=1)
        #online_monit =  IP_Alarm_Status.objects.filter(status=0)
        online_monit =  IP_Status.objects.filter(status=0)
        alarm_monit =  WEB_Alarm_Status.objects.filter(status=0)
        alarm_dnspod =  Dnspod_Alarm.objects.filter(status=0)
        alarm_dbip =  Port_Ip_Alarm_Status.objects.filter(status=0)
        alarm_port =  Port_Alarm_Status.objects.filter(status=0)
        ...
        monits = Monit.objects.filter(contact=contact)
        status = IP_Status.objects.filter(ip=ip)
        alarm = WEB_Alarm_Status.objects.filter(ip=ip)
        dnspods = Dnspod.objects.filter(ip=ip)
        dnspod = Dnspod_Alarm.objects.filter(ip=ip)
        ports = Port.objects.filter(ip=ip)
        port_ip = Port_Ip_Alarm_Status.objects.filter(ip=ip)
        port = Port_Alarm_Status.objects.filter(ip=ip)
        ...
        active_monits = monits.filter(rule=1)
        online_monit = status.filter(status=0)
        #online_monit = status.ip_status.filter(status=0)
        alarm_monit = alarm.ip_status.filter(status=0)
        alarm_dnspod = dnspod.filter(status=0)
        alarm_dbip = port_ip.filter(status=0)
        alarm_port = port.filter(status=0)
    ...
    if monits.count() == 0:
        percent_monit, percent_online_monit = '0%', '0%'
    else:
        percent_monit = format(active_monits.count() / monits.count(), '.0%')
        #percent_online_monit = format(online_monit.count() / status.count(), '.0%')
    if status.count() == 0:
        online_monit, percent_online_monit = '0%', '0%'
    else:
        percent_online_monit = format(online_monit.count() / status.count(), '.0%')
    if alarm.count() == 0:
        alarm_monit, percent_alarm_monit = '0%', '0%'
    else:
        percent_alarm_monit = format(alarm_monit.count() / monits.count(), '.0%')
    if dnspods.count() == 0:
        alarm_dnspod, percent_alarm_dnspod = '0%', '0%'
    else:
        percent_alarm_dnspod = format(alarm_dnspod.count() / dnspods.count(), '.0%')
    if ports.count() == 0:
        alarm_dbip, percent_alarm_dbip = '0%', '0%'
    else:
        percent_alarm_dbip = format(alarm_dbip.count() / ports.count(), '.0%')
    if ports.count() == 0:
        alarm_port, percent_alarm_port = '0%', '0%'
    else:
        percent_alarm_port = format(alarm_port.count() / ports.count(), '.0%')
