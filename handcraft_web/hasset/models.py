...
class Asset(models.Model):
    ...
    public_ip = models.IPAddressField(unique=True, null=True)
    manage_ip = models.IPAddressField(unique=True, null=True)
    #port = models.IntegerField(max_length=6)
    idc = models.ForeignKey(IDC)
    bis_group = models.ManyToManyField(BisGroup)
    dept = models.ManyToManyField(DEPT)
    #login_type = models.CharField(max_length=10, choices=LOGIN_TYPE_CHOICES, default='S')
...
class Adapter(models.Model):
    adapter = models.CharField(max_length=20, blank=True, null=True)
    ip = models.IPAddressField(unique=True)
    switch = models.CharField(max_length=20, blank=True, null=True)
