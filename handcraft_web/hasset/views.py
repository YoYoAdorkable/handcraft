...
def db_host_insert(host_info, username='', password=''):
    ...
    ip, public_ip, manage_ip, idc, group, dept, Manufacturer, os, sn, active, comment = host_info
    ...
    a = Asset(ip=ip, public_ip=public_ip, manage_ip=manage_ip,
              idc=idc,
              #port=port, login_type=jtype, idc=idc,
...
def db_host_update(host_info, username='', password=''):
    """ 修改主机时数据库操作函数 """
    ip, public_ip, manage_ip, idc, group, dept, Manufacturer, os, sn, active, comment, host = host_info
    #ip, public_ip, manage_ip, port, idc, jtype, group, dept, Manufacturer, os, sn, active, comment, host = host_info
    idc = IDC.objects.filter(id=idc)
    if idc:
        idc = idc[0]
    groups = get_host_groups(group)
...
def batch_host_edit(host_info, j_user='', j_password=''):
    """ 批量修改主机函数 """
    j_id, j_ip, j_public_ip, j_manage_ip, j_idc, j_group, j_dept, j_Manufacturer, j_os, j_sn, j_active, j_comment = host_info
...
def db_idc_delete(request, idc_id):
    ...
    default_idc = IDC.objects.get(id=idc_id)
...
def host_add(request):
    ...
    if request.method == 'POST':
        ...
        if is_super_user(request):
            ...
            host_info = [j_ip, j_public_ip, j_manage_ip, j_idc, j_group, j_dept, j_Manufacturer, j_os, j_sn, j_active, j_comment]
        ...
        if Asset.objects.filter(ip=str(j_ip)):
            ...
            return my_render('hasset/host_add.html', locals(), request)
        #if j_type == 'A':
...
def host_add_batch(request):
    ...
    if request.method == 'POST':
        ...
        for host in multi_hosts:
            if host == '':
                break
            j_ip, j_public_ip, j_manage_ip, j_idc, j_groups, j_depts, j_Manufacturer, j_os, j_sn, j_active, j_comment = host.split(',')
            #j_ip, j_public_ip, j_manage_ip, j_port, j_type, j_idc, j_groups, j_depts, j_Manufacturer, j_os, j_sn, j_active, j_comment = host.split(',')
            j_active = active_types[str(j_active)]
            #j_group = ast.literal_eval(j_groups)
            #j_dept = ast.literal_eval(j_depts)

            #if j_type not in ['SSH', 'TELNET']:
            ...
            group_ids, dept_ids = [], []
            for group_name in j_groups:
                group = BisGroup.objects.filter(name=j_groups)
            ...
            for dept_name in j_depts:
                dept = DEPT.objects.filter(name=j_depts)
            ...
            host_info = [j_ip, j_public_ip, j_manage_ip, j_idc, str(group_id), str(dept_id), j_Manufacturer, j_os, j_sn, j_active, j_comment]
            #host_info = [j_ip, j_public_ip, j_manage_ip, j_port, j_idc, j_type, str(group_id), str(dept_id), j_Manufacturer, j_os, j_sn, j_active, j_comment]
            #host_info = [j_ip, j_public_ip, j_manage_ip, j_port, j_idc, j_type, j_group, [j_dept], j_Manufacturer, j_os, j_sn, j_active, j_comment]
            db_host_insert(host_info)

        smg = u'批量添加添加成功'
        return my_render('hasset/host_add_multi.html', locals(), request)
...
def host_edit_batch(request):
    ...
    if request.method == 'POST':
        ...
        for i in range(int(len_table)):
            ...
            j_public_ip = "editable[" + str(i) + "][j_public_ip]"
            j_manage_ip = "editable[" + str(i) + "][j_manage_ip]"
            ...
            j_Manufacturer = "editable[" + str(i) + "][j_Manufacturer]"
            j_os = "editable[" + str(i) + "][j_os]"
            j_sn = "editable[" + str(i) + "][j_sn]"
            ...
            j_public_ip = request.POST.get(j_public_ip).strip()
            j_manage_ip = request.POST.get(j_manage_ip).strip()
            j_port = request.POST.get(j_port).strip()
            j_dept = request.POST.getlist(j_dept)
            j_idc = request.POST.get(j_idc).strip()
            j_type = request.POST.get(j_type).strip()
            j_group = request.POST.getlist(j_group)
            j_Manufacturer = request.POST.get(j_Manufacturer).strip()
            j_os = request.POST.get(j_os).strip()
            j_sn = request.POST.get(j_sn).strip()
            j_active = request.POST.get(j_active).strip()
            j_comment = request.POST.get(j_comment).strip()

            print j_id, j_ip, j_public_ip, j_manage_ip,j_idc,j_group,j_dept,j_comment

            host_info = [j_id, j_ip, j_public_ip, j_manage_ip, j_idc, j_group, j_dept, j_comment]
            #host_info = [j_id, j_ip, j_public_ip, j_manage_ip, j_port, j_idc, j_type, j_group, [j_dept], j_Manufacturer, j_os, j_sn, j_active, j_comment]
            #print host_info

            batch_host_edit(host_info)

        return HttpResponseRedirect('/hasset/host_list/')
...
def host_edit_common_batch(request):
    ...
    return my_render('hasset/host_list_common.html', locals(), request)
...
def host_list(request):
    ...
    if did:
        ...
        return my_render('hasset/host_list_nop.html', locals(), request)
        ...
        if is_super_user(request):
            ...
            return my_render('hasset/host_list.html', locals(), request)
...
def host_edit(request):
    ...
    if request.method == 'POST':
        ...
        #host_info = [j_ip, j_public_ip, j_manage_ip, j_port, j_idc, j_type, j_group, j_dept, j_Manufacturer, j_os, j_sn, j_active, j_comment]
        host_info = [j_ip, j_public_ip, j_manage_ip, j_idc, j_group, j_dept, j_Manufacturer, j_os, j_sn, j_active, j_comment, post]
        #host_info = [j_ip, j_public_ip, j_manage_ip, j_port, j_idc, j_type, j_group, j_dept, j_Manufacturer, j_os, j_sn, j_active, j_comment, post]
        #if j_type == 'A':
        #    j_user = request.POST.get('j_user')
        #    j_password = request.POST.get('j_password')
        ...
        return HttpResponseRedirect('/hasset/host_detail/?id=%s' % host_id)

    return my_render('hasset/host_edit.html', locals(), request)
...
def host_detail(request):
    ...
    return my_render('hasset/host_detail.html', locals(), request)
...
def idc_add(request):
    ...
    if request.method == 'POST':
        ...
        if IDC.objects.filter(name=j_idc):
            emg = u'该IDC已存在!'
            return my_render('hasset/idc_add.html', locals(), request)
        else:
...
def idc_list(request):
    ...
    contact_list, p, contacts, page_range, current_page, show_first, show_end = pages(posts, request)
    return my_render('hasset/idc_list.html', locals(), request)
...
def idc_edit(request):
    ...
    if request.method == 'POST':
        ...
        return HttpResponseRedirect('/hasset/idc_list/?id=%s' % idc_id)

    return my_render('hasset/idc_edit.html', locals(), request)
...
def idc_detail(request):
    ...
    return my_render('hasset/idc_detail.html', locals(), request)
...
def idc_del(request):
    ...
    return HttpResponseRedirect('/hasset/idc_list/')
...
def group_add(request):
    ...
    return my_render('hasset/group_add.html', locals(), request)
...
def group_list(request):
    ...
    contact_list, p, contacts, page_range, current_page, show_first, show_end = pages(posts, request)
    return my_render('hasset/group_list.html', locals(), request)
...
def group_edit(request):
    ...
    if request.method == 'POST':
        ...
        return HttpResponseRedirect('/hasset/group_list')

    return my_render('hasset/group_edit.html', locals(), request)
...
def group_detail(request):
    ...
    contact_list, p, contacts, page_range, current_page, show_first, show_end = pages(posts, request)
    return my_render('hasset/group_detail.html', locals(), request)
...
def group_del_host(request):
    ...
    return HttpResponseRedirect('/hasset/group_detail/?id=%s' % group.id)
...
def dept_host_ajax(request):
    ...
    return my_render('hasset/dept_host_ajax.html', locals(), request)
...
def show_all_ajax(request):
    ...
    if host:
        host = host[0]
    return my_render('hasset/show_all_ajax.html', locals(), request)
...
def host_search(request):
    ...
    return my_render('hasset/host_search.html', locals(), request)


@require_login
def host_port(request):
    """ 列出主机 """
    header_title, path1, path2 = u'查看主机', u'端口管理', u'查看主机'
    keyword = request.GET.get('keyword', '')
    dept_id = get_session_user_info(request)[3]
    dept = DEPT.objects.get(id=dept_id)
    did = request.GET.get('did', '')
    ...
    post_all = Adapter.objects.all().order_by('switch','port')
    post_keyword_all = Adapter.objects.filter(Q(adapter__contains=keyword) |
                                            Q(ip__contains=keyword) |
                                            Q(switch__contains=keyword) |
                                            Q(port__contains=keyword)).distinct().order_by('switch','port')
    if did:
        if is_common_user(request):
            return httperror(request, u'您无权查看!')
        ...
        if is_super_user(request):
            if keyword:
                ...
                posts = post_all
            contact_list, p, contacts, page_range, current_page, show_first, show_end = pages(posts, request)
            return my_render('hasset/host_port.html', locals(), request)
...
def port_search(request):
    ...
    login_types = {'S': 'SSH', 'T': 'TELNET'}
    dept = get_session_user_info(request)[5]
    post_all = Adapter.objects.filter(Q(ip__contains=keyword) |
                                    Q(switch__contains=keyword) |
                                    Q(port__contains=keyword) |
                                    Q(adapter__contains=keyword)).distinct().order_by('switch','port')
    if is_super_user(request):
        posts = post_all
        print posts
    elif is_group_admin(request):
    ...
    return my_render('hasset/port_search.html', locals(), request)
