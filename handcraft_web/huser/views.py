...
#def server_add_user(username, password):
...
def dept_add(request):
    ...
    return render_to_response('huser/dept_add.html', locals(), context_instance=RequestContext(request))
...
def dept_list(request):
    ...
    return render_to_response('huser/dept_list.html', locals(), context_instance=RequestContext(request))
...
def dept_detail(request):
    ...
    if not dept_id:
        return HttpResponseRedirect('/huser/dept_list/')
    dept = DEPT.objects.filter(id=dept_id)
    if dept:
        dept = dept[0]
        users = dept.user_set.all()
    return render_to_response('huser/dept_detail.html', locals(), context_instance=RequestContext(request))
...
def dept_edit(request):
    ...
    if request.method == 'GET':
        ...
        if dept:
            ...
            error = '部门不存在'
        return HttpResponseRedirect('/huser/dept_list/')
    return render_to_response('huser/dept_edit.html', locals(), context_instance=RequestContext(request))
...
def dept_user_ajax(request):
    ...
    return render_to_response('huser/dept_user_ajax.html', locals())
...
def group_add(request):
    ...
    return render_to_response('huser/group_add.html', locals(), context_instance=RequestContext(request))
...
def group_list(request):
    ...
    contact_list, p, contacts, page_range, current_page, show_first, show_end = pages(contact_list, request)
    return render_to_response('huser/group_list.html', locals(), context_instance=RequestContext(request))
...
def group_detail(request):
    ...
    return render_to_response('huser/group_detail.html', locals(), context_instance=RequestContext(request))
...
def group_del(request):
    ...
    return HttpResponseRedirect('/huser/group_list/')
...
def group_edit(request):
    ...
    if request.method == 'GET':
        ...
        return render_to_response('huser/group_edit.html', locals(), context_instance=RequestContext(request))
    else:
...
def user_add(request):
        ...
        try:
            ...
            try:
                user = db_add_user(username=username,
                                   ...
                                   name=name, email=email, phone=phone, dept=dept,
    ...
    return render_to_response('huser/user_add.html', locals(), context_instance=RequestContext(request))
...
def user_add_adm(request):
    ...
    if request.method == 'POST':
        ...
        try:
            if '' in [username, password, name, groups, is_active]:
                error = u'带*内容不能为空'
                raise AddError
            user = User.objects.filter(username=username)
            if user:
...
def user_list(request):
    ...
    did = request.GET.get('did', '')
    contact_list = User.objects.all().order_by('username')
    ...
    return render_to_response('huser/user_list.html', locals(), context_instance=RequestContext(request))
...
def user_list_adm(request):
    ...
    if gid:
        if not validate(request, user_group=[gid]):
            return HttpResponseRedirect('/huser/user_list/')
        user_group = UserGroup.objects.filter(id=gid)
        if user_group:
            user_group = user_group[0]
            contact_list = user_group.user_set.all()
...
def user_detail(request):
    ...
    if user:
        user = user[0]
        #monit_list_permed = monit_perm_list_api(user_id)
        monit_list_permed = monit_perm_list_api(user)
    ...
    return render_to_response('huser/user_detail.html', locals(), context_instance=RequestContext(request))
...
def user_edit(request):
    ...
    return render_to_response('huser/user_edit.html', locals(), context_instance=RequestContext(request))
...
def user_edit_adm(request):
    ...
    if request.method == 'GET':
        ...
        if not validate(request, user=[user_id], user_group=groups):
            return HttpResponseRedirect('/huser/user_edit/')
        if user_id:
            user = User.objects.filter(id=user_id)
            if user:
                user = user[0]
...
def profile(request):
    ...
    if not user_id:
        return HttpResponseRedirect('/')
    user = User.objects.get(id=user_id)
    return render_to_response('huser/profile.html', locals(), context_instance=RequestContext(request))
...
def chg_info(request):
    ...
    if request.method == 'POST':
        ...
        if '' in [name, password, email, phone]:
            error = '不能为空'
        ...
        if not error:
            ...
            user_set.update(name=name, password=password, email=email, phone=phone)
            msg = '修改成功'

    return render_to_response('huser/chg_info.html', locals(), context_instance=RequestContext(request))
